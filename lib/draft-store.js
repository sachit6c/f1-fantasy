...
export class DraftStore {
  ...
  /**
   * Updates the draft type before any picks have been made.
   * @param {string} newType - New draft type key (e.g. 'snake', 'linear', 'random_snake', 'full_pick')
   */
  updateDraftType(newType) {
    if (!this.draft) return;
    this.draft.config.draftType = newType;
    if (newType === 'random_snake') {
      this.draft.config.pickOrderSeed = Math.floor(Math.random() * 2);
    } else {
      delete this.draft.config.pickOrderSeed;
    }
    // Re-create rules with updated config
    this.rules = new DraftRules(this.draft.config);
    this.saveDraft();
    // Persist type preference so next session starts with the same type
    const p1 = this.draft.players[0]?.name || 'Player 1';
    const p2 = this.draft.players[1]?.name || 'Player 2';
    this.savePlayerNames(p1, p2, newType);
    this.notifyListeners();
  }
